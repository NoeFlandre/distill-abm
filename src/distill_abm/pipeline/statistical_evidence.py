...
def build_statistical_evidence(
    *,
    ...
    series_summaries = [
        _summarize_series(
            column=column,
            values=selected_frame[column].to_numpy(dtype=float),
            time_values=selected_frame[time_column].to_numpy(dtype=float) if time_column is not None else None,
            compression_tier=compression_tier,
        )
...
def _summarize_series(
    *,
    ...
    peaks, valleys = _detect_extrema(values=values, time_values=time_values, compression_tier=compression_tier)
    return StatisticalSeriesSummary(
        column=column,
        ...
        peaks=peaks,
        valleys=valleys,
        inflection_indices=_detect_inflections(values=values, compression_tier=compression_tier),
        rolling_mann_kendall=_rolling_mann_kendall(values=values, compression_tier=compression_tier),
        change_points=_detect_change_points(values=values, compression_tier=compression_tier),
        oscillation=_detect_oscillation(values=values, compression_tier=compression_tier),
    )
...
def _detect_extrema(
    *,
    ...
    try:
        ...
        peaks = _fallback_extrema(values=values, time_values=time_values, limit=max_items, valleys=False)
        valleys = _fallback_extrema(values=values, time_values=time_values, limit=max_items, valleys=True)
...
def _detect_inflections(*, values: np.ndarray, compression_tier: int) -> list[int]:
    try:
    ...
    return [int(value) for value in change_indices[:limit]]


def _rolling_mann_kendall(*, values: np.ndarray, compression_tier: int) -> dict[str, object]:
    try:
        ...
        rows.append(
            {
                ...
                "end_index": int(start + len(segment) - 1),
...
def _detect_change_points(*, values: np.ndarray, compression_tier: int) -> list[int]:
    try:
    ...
    try:
        ...
        result = [int(point) for point in algo.predict(pen=10)]
    except Exception:
        return []
...
def _detect_oscillation(*, values: np.ndarray, compression_tier: int) -> dict[str, object]:
    try:
        import pywt
    except Exception:
        return {"status": "unavailable", "reason": "PyWavelets not installed"}
    scales = np.arange(1, min(64, max(4, len(values) // 2)))
    if scales.size == 0:
        return {"status": "too_short"}
    try:
        coefficients, _frequencies = pywt.cwt(values, scales, "morl", sampling_period=1.0)
    ...
    return {
        ...
        "oscillation_indices": [int(index) for index in oscillation_indices[:limit]],
    }
