...
def test_execute_sync_results_bucket_command_rejects_empty_apply_sync(tmp_path: Path) -> None:
    source_root = tmp_path / "results"
    source_root.mkdir()
    (source_root / "README.md").write_text("pointer only\n", encoding="utf-8")

    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setattr("distill_abm.cli_actions.shutil.which", lambda _: "/usr/local/bin/hf")
        with pytest.raises(typer.BadParameter, match="refusing destructive bucket sync"):
            execute_sync_results_bucket_command(
                source_root=source_root,
                bucket_uri="hf://buckets/NoeFlandre/distill-abms-results",
                dry_run=False,
...
def test_execute_sync_results_bucket_command_allows_empty_apply_sync_when_overridden(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    source_root = tmp_path / "results"
