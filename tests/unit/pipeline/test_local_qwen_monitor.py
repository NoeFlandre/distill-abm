...
def test_collect_local_qwen_monitor_snapshot_characterizes_running_sample_case(tmp_path: Path) -> None:
    ...
    assert case.progress_detail == "context"
    assert case.completed_steps == 0
    assert case.total_steps == 2
    assert case.num_ctx == 16384
    assert case.max_tokens == 4096
...
def test_collect_local_qwen_monitor_snapshot_characterizes_failed_full_case_from_validation_state(
    tmp_path: Path,
) -> None:
    run_root = tmp_path / "runs" / "run_1"
    (tmp_path / "latest_run.txt").write_text(str(run_root), encoding="utf-8")
    ...
    (case_dir / "validation_state.json").write_text(
        json.dumps(
            {
                "context": {"status": "accepted", "error": None},
                "trends": {"1": {"status": "retry", "error": "boom"}},
            }
        ),
        encoding="utf-8",
    )
    ...
    assert case.error == "failed_trends=plot_01"
    assert case.progress_detail == "failed trend plot_01"
    assert case.completed_steps == 1
    assert case.total_steps == 2
...
def test_collect_local_qwen_monitor_snapshot_characterizes_suite_snapshot_from_nested_abm_statuses(
    tmp_path: Path,
) -> None:
    abm_output_root = tmp_path / "abms" / "fauna"
    abm_run_root = abm_output_root / "runs" / "run_1"
    abm_output_root.mkdir(parents=True, exist_ok=True)
    (abm_output_root / "latest_run.txt").write_text(str(abm_run_root), encoding="utf-8")
    failed_case = abm_run_root / "cases" / "01_case" / "03_outputs"
    completed_case_requests = abm_run_root / "cases" / "02_case" / "02_requests"
    completed_case_outputs = abm_run_root / "cases" / "02_case" / "03_outputs"
    failed_case.mkdir(parents=True, exist_ok=True)
    completed_case_requests.mkdir(parents=True, exist_ok=True)
    completed_case_outputs.mkdir(parents=True, exist_ok=True)
    (failed_case / "error.txt").write_text("boom", encoding="utf-8")
    (completed_case_requests / "trend_request.json").write_text(
        json.dumps(
            {
                "max_tokens": 1024,
                "prompt_length": 200,
    ...
    (completed_case_outputs / "trend_trace.json").write_text(
        json.dumps({"response": {"usage": {"total_tokens": 77}}}),
        encoding="utf-8",
    )
    (tmp_path / "suite_progress.json").write_text(
        json.dumps(
            {
                ...
                "current_abm": "fauna",
                "total_abms": 1,
                "completed_abm_count": 0,
                "failed_abm_count": 1,
                "abms": [{"abm": "fauna", "status": "running", "last_error": None}],
            }
        ),
        encoding="utf-8",
    )
    ...
    assert snapshot.mode == "suite"
    assert snapshot.failed_cases == 1
    assert snapshot.running_case_id == "fauna"
    case = snapshot.cases[0]
    assert case.status == "failed"
    assert case.completed_steps == 1
    assert case.total_steps == 2
    assert case.num_ctx is None
    assert case.max_tokens is None


def test_collect_local_qwen_monitor_snapshot_characterizes_tuning_trial_fallbacks(tmp_path: Path) -> None:
    trial_root = tmp_path / "trials" / "plot" / "num_ctx_8192" / "max_tokens_1024" / "cases"
    failed_outputs = trial_root / "01_case" / "03_outputs"
    pending_case = trial_root / "02_case"
    failed_outputs.mkdir(parents=True, exist_ok=True)
    pending_case.mkdir(parents=True, exist_ok=True)
    (failed_outputs / "error.txt").write_text("boom", encoding="utf-8")
    ...
    assert trial.label == "num_ctx_8192/max_tokens_1024"
    assert trial.error == "failed_cases=01_case"
    assert trial.num_ctx == 8192
    assert trial.max_tokens == 1024
